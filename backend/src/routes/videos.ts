...
const fileFilter = (req: Request, file: Express.Multer.File, cb: multer.FileFilterCallback) => {
    const allowedMimeTypes = [
        'video/mp4', 'video/webm', 'video/avi', 'video/mov', 'video/mkv', 'video/flv', 'video/x-matroska', 'application/octet-stream'
    ];
...
const screenshotFileFilter = (req: Request, file: Express.Multer.File, cb: multer.FileFilterCallback) => {
    const allowedMimeTypes = [
    ...
    if (allowedMimeTypes.includes(file.mimetype)) {
        ...
        cb(new Error('Invalid file type. Only image files are allowed for screenshot search.'));
    }
};
...
router.post('/upload', upload.single('movie'), async (req: Request, res: Response) => {
    try {
        if (!req.file) {
        ...
        res.status(500).json({ error: 'Failed to upload movie', details: error instanceof Error ? error.message : 'Unknown error' });
    }
});
